...
Ця програма демонструє ситуацію, коли користувач **може змінювати файл, не будучи його власником**, за умови, що він входить до групи, яка має права на запис до цього файлу.

**Пояснення:**
- Файл створюється з правами `rw-rw----` (тобто доступний для власника та групи).
- Власник файлу встановлюється як `user1`, а група — як `sharedgroup`.
- Інший користувач (`user2`), який входить у цю групу, має змогу змінювати вміст файлу, навіть не будучи його власником.

Програма створює файл `/tmp/shared_group_file.txt` та показує, які кроки слід виконати для встановлення відповідної групи і перевірки доступу з іншого облікового запису.

## Як запустити:

1. Скомпілюйте програму:
```bash
gcc -o group_write_demo group_write_demo.c
```

2. Запустіть від імені `user1` (власника файлу):
```bash
./group_write_demo
```

3. Виконайте наведені вивідні команди (від root):
```bash
sudo chown user1:sharedgroup /tmp/shared_group_file.txt
sudo chmod 660 /tmp/shared_group_file.txt
```

4. Перейдіть під інший обліковий запис (`user2`) і додайте текст до файлу:
```bash
echo "Додано user2." >> /tmp/shared_group_file.txt
cat /tmp/shared_group_file.txt
```

**Очікуваний результат:**

Файл буде містити обидва рядки: від `user1` та `user2`, попри те, що `user2` не є власником. Це можливо, бо він входить до групи з правами запису на файл.

```plaintext
Цей файл створено користувачем A (власником), група має доступ до запису.
Додано user2.
```

Таким чином, **груповий доступ із правами запису** — це один із способів змінювати файл без володіння ним.